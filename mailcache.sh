...
show_help() {
    ...
    echo "  --show-all-config  Show configuration for all applications"
    echo ""
    echo "Application-specific arguments:"
    echo "  --config           Edit configuration interactively"
    echo "  --show-config      Show current configuration"
    echo "  --task <name>      Run a specific task"
    echo "  --list-tasks       List available tasks for the selected app"
    echo ""
    echo "Examples:"
    echo "  $0 --show-all-config"
    echo "  $0 --app \"Password Manager\" --show-config"
    echo "  $0 --app \"Password Manager\" add imap server.com user"
    echo "  $0 --app \"Password Manager\" --config"
    echo "  $0 --app \"MailCache CLI\" --task sync-folder"
    echo "  $0 --app \"MailCache CLI\" --show-config"
    echo "  $0 --list"
    echo ""
